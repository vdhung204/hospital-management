...
@Service
@Transactional // Đảm bảo tính toàn vẹn khi insert nhiều bảng
public class AuthService {
    ...
    public LoginResponse registerPatient(PatientRegisterRequest req) {
        ...
        // 1. Check username
        if (userAccountRepository.findByUsername(req.getUsername()).isPresent()) {
            throw new AppException(401, "Tên đăng nhập đã tồn tại");
        }

        // --- CHECK UNIQUE BỆNH NHÂN ---
        if (req.getPhone() != null && patientRepository.existsByPhone(req.getPhone())) {
            throw new AppException(400, "Số điện thoại này đã được đăng ký bởi bệnh nhân khác");
        }
        if (req.getIdNumber() != null && patientRepository.existsByIdNumber(req.getIdNumber())) {
            throw new AppException(400, "Số CCCD/CMND này đã tồn tại trong hệ thống");
        }
        // ------------------------------
    ...
    public LoginResponse registerStaff(StaffRegisterRequest req) {
        ...
        // --- CHECK UNIQUE NHÂN VIÊN ---
        if (req.getPhone() != null && staffRepository.existsByPhone(req.getPhone())) {
            throw new AppException(400, "Số điện thoại nhân viên đã tồn tại");
        }
        if (req.getEmail() != null && staffRepository.existsByEmail(req.getEmail())) {
            throw new AppException(400, "Email nhân viên đã tồn tại");
        }
        if (req.getIdNumber() != null && staffRepository.existsByIdNumber(req.getIdNumber())) {
            throw new AppException(400, "Số CCCD/CMND nhân viên đã tồn tại");
        }
        // ------------------------------
