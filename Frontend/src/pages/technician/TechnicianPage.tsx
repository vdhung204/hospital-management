...
//         <div>
//             <h1 className="text-2xl font-bold text-gray-800">Khu vực kỹ thuật</h1>
//             <p className="text-gray-500">Danh sách chờ thực hiện cận lâm sàng</p>
//         </div>
...
//                 className={`flex items-center gap-2 px-6 py-2 rounded-md font-medium transition-all ${activeTab==='LAB' ? 'bg-blue-100 text-blue-700 shadow-sm' : 'text-gray-500 hover:bg-gray-50'}`}
//             >
//                 <FlaskConical size={18}/> Xét nghiệm
//             </button>
//             <button 
//                 onClick={() => setActiveTab('IMG')}
//                 className={`flex items-center gap-2 px-6 py-2 rounded-md font-medium transition-all ${activeTab==='IMG' ? 'bg-purple-100 text-purple-700 shadow-sm' : 'text-gray-500 hover:bg-gray-50'}`}
//             >
//                 <ImageIcon size={18}/> CĐHA (X-Quang/Siêu âm)
//             </button>
//         </div>
//       </div>
...
//                 <input 
//                     type="text" 
//                     placeholder="Tìm tên dịch vụ, bệnh nhân..." 
//                     className="w-full pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
//                     value={searchTerm}
//                     onChange={e => setSearchTerm(e.target.value)}
//                 />
//             </div>
//             <button onClick={loadData} className="px-4 py-2 bg-gray-100 rounded-lg hover:bg-gray-200 text-gray-700 font-medium">Làm mới</button>
//          </div>
...
//         {/* HEADER: Nền xám nhạt, chữ đậm, uppercase */}
//         <thead className="bg-gray-100 text-gray-600 font-bold uppercase text-xs tracking-wider">
//             <tr>
//                 <th className="px-6 py-4 w-40">Thời gian / ID</th>
//                 <th className="px-6 py-4 w-64">Thông tin bệnh nhân</th>
...
//                 <tr><td colSpan={5} className="text-center py-16 text-gray-400 italic">Không có yêu cầu nào đang chờ.</td></tr>
//             ) : (
//                 filteredItems.map(item => (
//                     <tr key={item.id} className="hover:bg-blue-50/50 transition-colors group">
                        
//                         {/* Cột 1: Thời gian & ID Phiếu */}
...
//                                     {new Date(item.orderedAt).toLocaleDateString('vi-VN')}
//                                 </span>
//                                 <span className="inline-block bg-gray-100 text-gray-500 text-[10px] px-1.5 py-0.5 rounded border border-gray-200 w-fit font-mono">
//                                     #{item.id}
//                                 </span>
//                             </div>
//                         </td>

//                         {/* Cột 2: Bệnh nhân (Tên, Mã, Ngày sinh) */}
//                         <td className="px-6 py-4 align-top">
//                             <div className="flex gap-3">
//                                 {/* Avatar chữ cái */}
//                                 <div className={`w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center text-white font-bold shadow-sm ${
//                                     item.patientGender === 'Nam' || item.patientGender === 'M' ? 'bg-blue-500' : 'bg-pink-500'
//                                 }`}>
//                                     {item.patientName ? item.patientName.charAt(0) : '?'}
//                                 </div>
                                
//                                 <div>
//                                     <p className="font-bold text-gray-800 text-base leading-tight mb-1">{item.patientName}</p>
...
//                                             <User size={10} /> {item.patientGender}
//                                         </span>
//                                         {/* Hiển thị Ngày sinh */}
//                                         <span className="flex items-center gap-1 bg-gray-100 px-1.5 py-0.5 rounded">
//                                             <Clock size={10} /> {item.patientDob ? new Date(item.patientDob).toLocaleDateString('vi-VN') : '--/--/----'}
//                                         </span>
//                                     </div>
//                                 </div>
//                             </div>
...
//                                 <div className="mt-1">
//                                     {/* Icon phân loại */}
//                                     {item.itemType === 'LAB_TEST' ? <FlaskConical size={16} className="text-blue-500"/> : <ImageIcon size={16} className="text-purple-500"/>}
//                                 </div>
//                                 <div>
//                                     <p className="font-bold text-gray-800">{item.serviceItemName}</p>
//                                     <span className={`text-[10px] px-2 py-0.5 rounded border ${
//                                         item.itemType === 'LAB_TEST' ? 'bg-blue-50 text-blue-600 border-blue-100' : 'bg-purple-50 text-purple-600 border-purple-100'
//                                     }`}>
...
//                         {/* Cột 4: Bác sĩ chỉ định */}
//                         <td className="px-6 py-4 align-top">
//                             <div className="flex items-center gap-2">
//                                 <div className="w-6 h-6 rounded-full bg-gray-200 flex items-center justify-center text-[10px] font-bold text-gray-500">BS</div>
//                                 <span className="text-sm font-medium text-gray-700">{item.doctorName}</span>
//                             </div>
//                         </td>

//                         {/* Cột 5: Nút bấm (Căn phải) */}
//                         <td className="px-6 py-4 align-middle text-right">
//                             <button 
//                                 onClick={() => setSelectedItem(item)}
//                                 className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold text-sm shadow-md transition-all hover:-translate-y-0.5 active:scale-95"
//                             >
//                                 <FileText size={16}/> Nhập KQ
//                             </button>
//                         </td>
//                     </tr>
//                 ))
